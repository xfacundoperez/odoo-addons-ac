...
class BM_ResCompany(models.Model):
    ...
    def _compute_address_name(self):
        for official in self:
            ...
            if _dir:
                ...
                official.address_name = 'S/D'
            # Compute days
            self._compute_day_week()

    exploitation = fields.Char(string='Explotación', required=True)
    years = fields.Selection(LIST_YEARS, string='Año',
                             default=str(fields.Datetime.now().year))
    ...
    currency_ids = fields.Many2many(
        'res.currency', 'company_allowed_currency_rel', 'id', string='Monedas permitidas', store=True)
    company_code = fields.Char('Código de la Empresa')
    ...
    daytime_overtime = fields.Float('Hora extraordinaria diurna', default=0)
    night_overtime = fields.Float('Hora extraordinaria nocturna', default=0)
    holiday_overtime = fields.Float('Hora extraordinaria feriado', default=0)
...
class BM_ResCompanySchedules(models.Model):
    ...
    checkin = fields.Selection(
        LIST_HOUR, 'Entrada', default='07:30', required=True)
    checkout = fields.Selection(
        LIST_HOUR, 'Salida', default='17:30', required=True)
